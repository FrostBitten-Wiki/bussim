        ...
        if(el.getAttribute("processed") != "true") {
            ...
            for(i in textArray) {
                ...
                if(!ignore.includes(parseInt(i))) {
                    processedString += `<${nodeName}-child style='display: inline-block; overflow: hidden; font-size: inherit; animation: wave 1.4s ease-in-out infinite; animation-delay: ${offset}ms; ${extraStyles}'>${textArray[i]}</${nodeName}-child>`
                    offset += 75
                }
            }
